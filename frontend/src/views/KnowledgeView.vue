...
const activeTab = ref<'overview' | 'documents' | 'retrieval' | 'chat' | 'settings'>('overview')
...
function selectKnowledgeTab(tab: typeof activeTab.value) {
  activeTab.value = tab
}
...
<template>
  <main class="workspace-shell">
    ...
    <section v-else class="workspace-grid">
      ...
      <section class="panel content-panel"><div class="section-heading content-heading"><div><p class="eyebrow">KNOWLEDGE BASE WORKSPACE</p><h2>{{ knowledgeBase ? knowledgeBase.name : '当前知识库' }}</h2><span v-if="knowledgeBase" class="content-subtitle">{{ knowledgeBase.description || '团队知识资产与 AI 问答空间' }}</span></div><div class="knowledge-selector"><select v-model="knowledgeBaseId" :disabled="loading || !knowledgeBases.length" @change="handleKnowledgeBaseChange"><option value="" disabled>{{ knowledgeBases.length ? '选择知识库' : '暂无知识库' }}</option><option v-for="item in knowledgeBases" :key="item.id" :value="String(item.id)">{{ item.name }}</option></select><span v-if="knowledgeBase" class="status-pill active">{{ knowledgeBase.status }}</span></div></div>
        <div v-if="knowledgeBase" class="knowledge-content"><div class="knowledge-meta"><span>知识库 #{{ knowledgeBase.id }}</span><span>工作空间 #{{ knowledgeBase.workspaceId }}</span></div><nav class="knowledge-tabs" aria-label="Knowledge base sections"><button v-for="tab in [{ key: 'overview', label: 'Overview' }, { key: 'documents', label: 'Documents' }, { key: 'retrieval', label: 'Retrieval' }, { key: 'chat', label: 'Chat' }, { key: 'settings', label: 'Settings' }]" :key="tab.key" class="knowledge-tab" :class="{ active: activeTab === tab.key }" type="button" @click="selectKnowledgeTab(tab.key as typeof activeTab)">{{ tab.label }}</button></nav><div class="knowledge-overview-strip"><div><span>STATUS</span><strong>{{ knowledgeBase.status }}</strong></div><div><span>DOCUMENTS</span><strong>{{ documents.length }}</strong></div><div><span>EMBEDDING</span><strong>text-embedding-v4</strong></div><div><span>ACTIVE VIEW</span><strong>{{ activeTab }}</strong></div></div><div class="upload-zone"><div class="upload-icon">↑</div><div><strong>{{ selectedFile ? selectedFile.name : '上传一份资料' }}</strong><span>{{ selectedFile ? '已选择，准备开始解析' : '支持 TXT、MD、CSV、PDF、DOCX' }}</span></div><label class="file-button">选择文件<input type="file" accept=".txt,.md,.csv,.pdf,.docx" @change="selectFile" /></label><button class="primary-button" :disabled="loading || !selectedFile" type="button" @click="upload">{{ loading ? '上传中…' : '上传并解析' }}</button></div>
